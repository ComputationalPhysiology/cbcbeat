...
class Tentusscher_panfilov_2006_M_cell(CardiacCellModel):
    ...
    def _I(self, v, s, time):
        ...
        i_CaL = (
            ...
            * (F * F)
            * (-15 + V)
            * (0.25 * Ca_ss * ufl.exp(F * (-30 + 2 * V) / (R * T)) - Ca_o)
            * d
            * f
            * f2
            * fCass
            / (R * T * (-1 + ufl.exp(F * (-30 + 2 * V) / (R * T))))
        )
    ...
    def F(self, v, s, time=None):
        ...
        tau_f = (
            20
            + 1102.5 * ufl.exp(-((27 + V) * (27 + V)) / 225)
            + 180 / (1 + ufl.exp(3 + V / 10))
            + 200 / (1 + ufl.exp(13 / 10 - V / 10))
        )
        ...
        tau_f2 = (
            80 / (1 + ufl.exp(3 + V / 10))
            + 562 * ufl.exp(-((27 + V) * (27 + V)) / 240)
            + 31 / (1 + ufl.exp(5 / 2 - V / 10))
        )
        ...
        Ca_ss_bufss = 1.0 / (
            1 + Buf_ss * K_buf_ss / ((Ca_ss + K_buf_ss) * (Ca_ss + K_buf_ss))
        )
        F_expressions[13] = (
            i_xfer
        ...
        F_expressions[14] = (
            -Cm * i_CaL / (2 * F * V_ss) - V_c * i_xfer / V_ss + V_sr * i_rel / V_ss
        ) * Ca_ss_bufss
