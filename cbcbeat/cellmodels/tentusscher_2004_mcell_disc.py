...
class Tentusscher_2004_mcell_disc(CardiacCellModel):
    ...
    def F(self, v, s, time=None):
        ...
        alpha_h = ufl.conditional(
            ufl.lt(V, -40), 4.43126792958e-07 * ufl.exp(-0.147058823529 * V), 0
        )
        beta_h = ufl.conditional(
            ufl.lt(V, -40),
            2.7 * ufl.exp(0.079 * V) + 310000 * ufl.exp(0.3485 * V),
            0.77 / (0.13 + 0.0497581410839 * ufl.exp(-0.0900900900901 * V)),
        )
        ...
        beta_j = ufl.conditional(
            ufl.lt(V, -40),
            0.02424
            * ufl.exp(-0.01052 * V)
            / (1 + 0.0039608683399 * ufl.exp(-0.1378 * V)),
            0.6 * ufl.exp(0.057 * V) / (1 + 0.0407622039784 * ufl.exp(-0.1 * V)),
        )
        ...
        g_inf = ufl.conditional(
            ufl.lt(Ca_i, 0.00035),
            1.0 / (1 + 5.43991024148e20 * ufl.elem_pow(Ca_i, 6)),
            1.0 / (1 + 1.9720198874e55 * ufl.elem_pow(Ca_i, 16)),
        )
        d_g = (-g + g_inf) / tau_g
        F_expressions[13] = ufl.conditional(
            ufl.And(ufl.gt(V, -60), ufl.gt(g_inf, g)), 0, d_g
        )
        ...
        F_expressions[15] = (
            Cm * (2 * i_NaK - i_Ks - i_Stim - i_Kr - i_to - i_p_K - i_K1) / (F * V_c)
        )
